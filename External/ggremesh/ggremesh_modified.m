function [Fn,Vn]=ggremesh_modified(varargin)
...
%Setting pathnames
pathNameLib=fullfile(fileparts(fileparts(mfilename('fullpath'))),'ggremesh','geogram');

%Create temporary file name
inputFileName=fullfile(pathNameLib,'temp.obj');
outputFileName=fullfile(pathNameLib,'temp_out.obj');

%Create runName for binary
compString=computer; 
switch compString
    case 'PCWIN64' %Windows 64-bit
        pathNameTetGenFile=fullfile(pathNameLib,'win64');
        runName=fullfile(pathNameTetGenFile,'vorpalite.exe');
    case 'GLNXA64' %Linux 64-bit       
        pathNameTetGenFile=fullfile(pathNameLib,'lin64');
        runName=fullfile(pathNameTetGenFile,'vorpalite');   
    case 'MACI64'  %MAC 64-bit      
        pathNameTetGenFile=fullfile(pathNameLib,'mac64');
        runName=fullfile(pathNameTetGenFile,'vorpalite');
    otherwise
